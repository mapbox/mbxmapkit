...
A key concept to understand is that MBXMapKit, as of version 0.3.0, is designed to add features to Apple's MKMapKit rather than to replace or encapsulate it. MBXMapKit versions 0.1.0 through 0.2.1 encapsulated MKMapKit by managing an instance of MKMapView, but that turned out to be impractical for apps which needed more than a minimal level of control over the map's configuration.
